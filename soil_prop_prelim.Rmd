...
NOTE: This represents an intermediate step and will likely be heavily edited or removed because the FINAL GOAL is to plot ALL available pairs, with means and SE overlay. 
...
#Step 2: (this code works but the colors are gross)
...
O_plot.FUN <- function(data){
  Oplot <- ggplot(data, aes(x = O, y = A)) +
  scale_fill_viridis(discrete = TRUE) +
  geom_point(position = position_dodge(),
  color = OA_means.df$treatment,
  width = 0.2)
  Oplot <- Oplot + ggtitle("Plot of O horizon vs. A horizon depths (cm) by site") +
  ...
  Oplot <- Oplot +
    labs(x= "O horizon depth (cm)", y = "A horizon depth (cm)", size = rel(1.8)) +
...
O_plot.FUN(OA_means.df) #run the function
?geom_point

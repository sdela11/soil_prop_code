...
OA_means.df <- data.frame(v1=O,v2=A)
colnames(OA_means.df) = c("O","A")
rownames(OA_means.df) = c("C2A","C2B","C5A","C5B","D2A","D2B","D5A","D5B")
...
#Step 2:
qplot(O, A, data = OA_means.df, label = rownames(OA_means.df), 
      geom=c("point", "text"),
      hjust=0, vjust=0)
